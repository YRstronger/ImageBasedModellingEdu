...
public:
    ...
    math::Vec2d projection(math::Vec3d const & p3d){
        ...
        math::Vec2d p;
		// 世界坐标系到相机坐标系
		math::Matrix<double, 3, 3> rotationW2C(R_);   //使用math::matrix 的explicit构造函数
		math::Vec3d t(t_);                            //使用math::vector 的explicit构造函数
		auto cameraPosition = rotationW2C * p3d + t;      //（xc,yc,zc）=R*p3d+t
        
		//相机坐标系到相平面
		cameraPosition = cameraPosition / cameraPosition[2];   //利用zc归一化

		// 径向畸变过程
		double r2 = pow(cameraPosition[0], 2) + pow(cameraPosition[1], 2);  //计算半径
		double distort_ratio = 1 + dist_[0] * r2 + dist_[1] * r2*r2;      //计算畸变参数

		// 图像坐标系到屏幕坐标系
		p[0] = f_ * distort_ratio*cameraPosition[0] + c_[0];
		p[1] = f_ * distort_ratio*cameraPosition[1] + c_[1];
        return p;
        /**  Reference
        // 世界坐标系到相机坐标系
    ...
    math::Vec3d pos_in_world(){
		...
		math::Matrix<double, 3, 3> rotationW2C(R_);   //使用math::matrix 的explicit构造函数
		math::Vec3d t(t_);                            //使用math::vector 的explicit构造函数
		auto cameraPosition = -rotationW2C.transpose() * t;      //（xo,yo,zo）=R^T*t
